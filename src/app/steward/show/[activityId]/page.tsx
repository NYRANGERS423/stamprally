...
export default async function StewardShow({
  params,
}: {
  params: Promise<{ activityId: string }>;
}) {
  const { activityId } = await params;
  await requireSteward("stamp", `/steward/show/${activityId}`);

  const activity = await db.activity.findUnique({
    where: { id: activityId },
    include: { event: { select: { id: true, name: true, active: true } } },
  });
  if (!activity || !activity.active || !activity.event.active) notFound();
  ...
  const backHref = `/steward/events/${activity.event.id}`;
    ...
    <>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col items-center px-4 py-6 sm:px-6">
        ...
        <p className="mt-4 text-center text-xs text-stone-500 dark:text-stone-400">
          Show this on your phone for attendees to scan. Their stamp gets
          recorded against their own passport.
        </p>
      </main>
    </>
  );
